...
class MessageCallsTest extends MandrillSpec {
  ...
  Await.result({
    actualClient match {
      case Some(client) =>
        client.messagesSend(MSendMessage(message = validMessage)).map { testResponse =>
          testResponse.toOption.flatMap(_.headOption) match {
            case Some(testMessageResponse) =>
              "Actual message calls" should "send a valid message" taggedAs ActualAPICall in {
                whenReady(client.messagesSend(MSendMessage(message = validMessage)).map { res =>
                  Thread.sleep(5000) // Waiting for 5 seconds to avoid the message creation race exception
                  res
                }, defaultTimeout) {
                  case Success(res) =>
                    res.headOption match {
                      case r@Some(mres) =>
                        mres.status shouldBe "sent"
              ...
              it should "load info about the message we sent" taggedAs ActualAPICall in {
                whenReady(client.messagesInfo(MMessageInfo(id = testMessageResponse._id)), defaultTimeout) {
                  case Success(res) =>
                    ...
                    res.subject shouldBe validMessage.subject
                  case Failure(t) => fail(t)
                }
              }
            ...
            case None => "Actual message calls" should "get a test message response to use for tests" taggedAs ActualAPICall in {
              fail("Unable to get the test message response for further message tests")
            }
          }
        }
